...
void Utilities::pauseAllInNode(cocos2d::CCNode * n){
    CCArray* a = n->getChildren();
    CCObject* obj;
    CCARRAY_FOREACH(a, obj) {
        Utilities::pauseAllInNode((CCNode*)obj);
    }
    n->pauseSchedulerAndActions();
}

void Utilities::resumeAllInNode(cocos2d::CCNode * n){
    CCArray* a = n->getChildren();
    CCObject* obj;
    CCARRAY_FOREACH(a, obj) {
        Utilities::resumeAllInNode((CCNode*)obj);
    }
...
void Utilities::pushLog(std::string content){
    ...
    CCLabelTTF * label = CCLabelTTF::create(content.c_str(), "fonts/myriadprobold.ttf", 26, CCSizeZero, kCCTextAlignmentCenter, kCCVerticalTextAlignmentCenter);
    label->setAnchorPoint( ccp(0.5f, 0));
    label->setPosition( ccp(visibleSize.width * 0.5f, 20));
//    label->enableOutline(Color4B::BLACK, 2);
//    label->setAdditionalKerning(-2);
    label->setColor(ccc3(255, 77, 30));
    label->setOpacity(0);
    label->setScale(1.6f);
    ...
    CCFiniteTimeAction * action = CCSequence::create(
                                                 ...
                                                 CCCallFunc::create(label, callfunc_selector(CCLabelTTF::removeFromParent)),
                                                 NULL
                                                 );
